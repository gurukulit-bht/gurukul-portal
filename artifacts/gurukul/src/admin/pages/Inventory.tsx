...
import { Plus, Edit2, Trash2, Check, X, Search, AlertTriangle, RefreshCw } from "lucide-react";
...
export default function Inventory() {
  ...
  function openEdit(item: InventoryItem) { setEditing(item); setForm({ name: item.name, category: item.category, dateProcured: item.dateProcured, quantityProcured: item.quantityProcured, currentStock: item.currentStock, reorderLevel: item.reorderLevel, lastReplenishment: item.lastReplenishment, vendor: item.vendor, remarks: item.remarks }); setShowModal(true); }
  ...
  function handleSave() {
    ...
    if (editing) {
      ...
      setItems(prev => [...prev, { ...form, id: Math.max(...prev.map(i => i.id)) + 1 }]);
    }
    setShowModal(false);
  }

  function handleDelete(id: number) { setItems(prev => prev.filter(i => i.id !== id)); setDeleteConfirm(null); }

  function handleReplenish() {
    if (!replenishItem || replenishQty <= 0) return;
    setItems(prev => prev.map(i => i.id === replenishItem.id
      ? { ...i, currentStock: i.currentStock + replenishQty, lastReplenishment: new Date().toISOString().split("T")[0] }
      : i
    ));
    setReplenishItem(null); setReplenishQty(0);
  }
              ...
              {filtered.map(item => {
                ...
                return (
                  <tr key={item.id} className={`border-b border-border/50 hover:bg-gray-50 transition-colors ${item.currentStock <= item.reorderLevel ? "bg-orange-50/30" : ""}`}>
                    ...
                    <td className="px-4 py-3">
                      <div className="text-sm font-bold text-secondary text-center">{item.currentStock}</div>
                    </td>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl">
            ...
            <div className="p-6 border-t border-border flex justify-end gap-3">
              <Button variant="outline" onClick={() => setReplenishItem(null)} className="rounded-xl">Cancel</Button>
              <Button onClick={handleReplenish} className="rounded-xl" disabled={replenishQty <= 0}>Confirm Replenishment</Button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 border-t border-border flex justify-end gap-3">
              ...
              <Button onClick={handleSave} className="rounded-xl" disabled={!form.name.trim()}>{editing ? "Save Changes" : "Add Item"}</Button>
            </div>
          </div>
        </div>
      )}
