...
import { FileText, Download, Eye, Filter, Search, BookOpen, FileSpreadsheet, BookMarked, ClipboardList, Tag, Layers } from "lucide-react";
...
type Doc = {
  ...
  course: string; level: string; section?: string;
  fileType: "PDF" | "DOCX" | "XLSX";
  size: string; uploadedBy: string; uploadedOn: string;
};

// These are demo/mock documents. In a real backend, these would come from the DB.
const MOCK_DOCS: Doc[] = [
  ...
  { id: 14, title: "Gujarati Level 1 – Syllabus AY 2025-26",        type: "Syllabus",         course: "Gujarati", level: "Level 1", fileType: "PDF",  size: "1.0 MB", uploadedBy: "Sri Ramesh Nair",      uploadedOn: "2025-08-07" },
...
type LevelRow   = { id: number; level: number; className: string; sections: { id: number; sectionName: string }[]; };
...
export default function CourseDocuments() {
  ...
  const filtered = useMemo(() => {
    return MOCK_DOCS.filter(d => {
      const matchCourse = !selectedCourse || d.course.toLowerCase() === selectedCourse.name.toLowerCase();
      const matchLevel  = !selectedLevel  || d.level === selectedLevel.className || d.level === "All";
      const matchType   = selectedType === "All Types" || d.type === selectedType;
      const matchSearch = !search || d.title.toLowerCase().includes(search.toLowerCase());
      return matchCourse && matchLevel && matchType && matchSearch;
    });
  }, [selectedCourse, selectedLevel, selectedType, search]);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Filters */}
      <div className="bg-white rounded-xl shadow-sm border border-border p-4 space-y-3">
        ...
        <div className="flex flex-wrap gap-3 items-center">
          ...
          <select
            ...
            onChange={e => { setSelectedCourseId(e.target.value ? Number(e.target.value) : null); setSelectedLevelId(null); }}
          ...
          {/* Level filter — dynamic, shown when course selected */}
          {selectedCourse && (
          ...
          <div className="flex items-center gap-2 flex-wrap text-xs">
            ...
            {selectedLevel && (
              <span className="bg-blue-50 text-blue-700 border border-blue-200 px-2 py-0.5 rounded-full flex items-center gap-1">
                <Layers className="w-3 h-3" /> {selectedLevel.className}
              </span>
            )}
          </div>
        )}
