...
import { Edit2, Check, X, Users, BookOpen, Loader2 } from "lucide-react";
        ...
        <>
          ...
          <div className="bg-white rounded-2xl border border-border overflow-hidden">
            <div className="px-6 py-4 border-b border-border bg-gray-50 flex items-center justify-between">
              ...
              <span className="text-xs text-muted-foreground">Click edit to update schedule, capacity, or enrollment</span>
            </div>
                ...
                <div key={level.id} className="px-6 py-4 hover:bg-gray-50 transition-colors">
                  {editingLevel?.levelId === level.id ? (
                    <div className="space-y-3">
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        <div className="space-y-1">
                          ...
                          <Input value={editForm.schedule ?? ""} onChange={(e) => setEditForm((f) => ({ ...f, schedule: e.target.value }))} className="rounded-lg h-8 text-sm" />
                        </div>
                        <div className="space-y-1">
                          <Label className="text-xs">Capacity</Label>
                          <Input type="number" min={0} value={editForm.capacity ?? 0} onChange={(e) => setEditForm((f) => ({ ...f, capacity: +e.target.value }))} className="rounded-lg h-8 text-sm" />
                        </div>
                        <div className="space-y-1">
                          <Label className="text-xs">Enrolled</Label>
                          <Input type="number" min={0} value={editForm.enrolled ?? 0} onChange={(e) => setEditForm((f) => ({ ...f, enrolled: +e.target.value }))} className="rounded-lg h-8 text-sm" />
                        </div>
                        <div className="space-y-1">
                          <Label className="text-xs">Status</Label>
                          <select value={editForm.status ?? "Active"} onChange={(e) => setEditForm((f) => ({ ...f, status: e.target.value as "Active" | "Inactive" }))} className="w-full border border-input rounded-lg px-2 py-1 text-sm bg-white h-8">
                            <option>Active</option><option>Inactive</option>
                          </select>
                        </div>
                      </div>
                      <div className="flex gap-2">
                        <Button size="sm" onClick={saveLevel} className="rounded-lg gap-1.5 h-7 text-xs" disabled={saving}>
                          {saving ? <Loader2 className="w-3 h-3 animate-spin" /> : <Check className="w-3 h-3" />} Save
                        </Button>
                        <Button size="sm" variant="outline" onClick={() => setEditingLevel(null)} className="rounded-lg h-7 text-xs" disabled={saving}>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <button onClick={() => openEditLevel(level)} className="w-7 h-7 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 flex items-center justify-center transition-colors shrink-0">
