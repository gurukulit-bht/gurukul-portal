...
import { mockTeachers, Teacher } from "../mockData";
import { Plus, Edit2, Trash2, Check, X, Search, Phone, Mail } from "lucide-react";
...
export default function Teachers() {
  ...
  function openEdit(t: Teacher) { setEditing(t); setForm({ name: t.name, email: t.email, phone: t.phone, assignedCourse: t.assignedCourse, assignedLevel: t.assignedLevel, timing: t.timing, status: t.status }); setError(""); setShowModal(true); }
  ...
  function handleSave() {
    ...
    if (editing) {
      setTeachers(prev => prev.map(t => t.id === editing.id ? { ...form, id: editing.id } : t));
    } else {
      setTeachers(prev => [...prev, { ...form, id: Math.max(...prev.map(t => t.id)) + 1 }]);
    }
    setShowModal(false);
  }

  function handleDelete(id: number) { setTeachers(prev => prev.filter(t => t.id !== id)); setDeleteConfirm(null); }
                    ...
                    <div className="flex gap-2">
                      ...
                      {deleteConfirm === t.id ? (
                        <div className="flex gap-1">
                          <button onClick={() => handleDelete(t.id)} className="w-7 h-7 rounded-lg bg-red-500 text-white hover:bg-red-600 flex items-center justify-center transition-colors"><Check className="w-3 h-3" /></button>
                          <button onClick={() => setDeleteConfirm(null)} className="w-7 h-7 rounded-lg bg-gray-100 hover:bg-gray-200 flex items-center justify-center transition-colors"><X className="w-3 h-3" /></button>
                        </div>
                      ) : (
                        <button onClick={() => setDeleteConfirm(t.id)} className="w-7 h-7 rounded-lg bg-red-50 text-red-500 hover:bg-red-100 flex items-center justify-center transition-colors">
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 border-t border-border flex justify-end gap-3">
              ...
              <Button onClick={handleSave} className="rounded-xl">{editing ? "Save Changes" : "Add Teacher"}</Button>
            </div>
          </div>
        </div>
      )}
